...
describe('Katalog- und Wechselinvarianten der Erweiterung',()=>{
 it('enthält genau elf eindeutige und vollständige Ködertypen',()=>{expect(lures).toHaveLength(11);expect(new Set(lures.map(lure=>lure.id)).size).toBe(11);expect(lures.every(lure=>lure.sizes.length&&lure.depths.length&&lure.mounting&&lure.guidance&&lure.sizeRangesCm&&lure.presentations?.length)).toBe(true)})
 it('erstellt für jeden neuen Typ Farbhilfe und drei Wechselphasen',()=>{const conditions:Conditions={...base,depth:'unknown',waterTemperature:'warm',activity:{status:'observed',signs:['surfaceActivity','huntingPerch']}};const decision=createRecommendationDecision(conditions,lures.map(lure=>({targetFish:'perch',lureTypeId:lure.id,sizes:[...lure.sizes]})));const allRanked=[...decision.expertRanking,decision.practicalPrimary,decision.bestMissing].filter(Boolean);expect(allRanked.every(item=>item!.colorGuidance.examples.length===4&&item!.switchPlan.length===3)).toBe(true)})
