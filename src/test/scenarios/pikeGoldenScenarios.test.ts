...
describe('24 Hecht-Golden-Szenarien',()=>{
 ...
 it('lässt Bestand das Fachranking niemals verändern',()=>{const empty=createRecommendationDecision(base,[]).expertRanking;const stocked=createRecommendationDecision(base,pikeLures.map(lure=>({targetFish:'pike' as const,lureTypeId:lure.id,sizes:['large' as const]}))).expertRanking;expect(stocked).toEqual(empty)})
 it('ignoriert Barschbestand und nutzt eine abweichende vorhandene Hechtgröße als Kompromiss',()=>{const result=createRecommendationDecision(base,[{targetFish:'perch',lureTypeId:'jig',sizes:['large']},{targetFish:'pike',lureTypeId:'spinner',sizes:['small']}]).practicalPrimary;expect(result?.setup.lure.id).toBe('spinner');expect(result?.inventoryFit).toEqual({preferredSize:'large',selectedSize:'small',exact:false})})
