...
const initial=(fish:TargetFish):Conditions=>({targetFish:fish,waterType:'lake',season:'summer',timeOfDay:'day',turbidity:'slightly_turbid',depth:'medium',waterTemperature:'unknown',light:'unknown',activity:{status:'unknown',signs:[]},vegetation:'unknown',observedStructure:[],pikeSafetyConfirmed:fish==='pike'?false:undefined})
...
function SituationForm({fish}:{fish:TargetFish}){const[conditions,setConditions]=useState(()=>initial(fish));const navigate=useNavigate();const select=(key:keyof typeof choices,value:string)=>setConditions(current=>({...current,[key]:value}));const toggleStructure=(value:ObservableStructure)=>setConditions(current=>({...current,observedStructure:current.observedStructure.includes(value)?current.observedStructure.filter(item=>item!==value):[...current.observedStructure,value]}));const setActivityStatus=(status:'unknown'|'none')=>setConditions(current=>({...current,activity:{status,signs:[]}}));const toggleActivity=(value:ActivitySign)=>setConditions(current=>{const signs=current.activity.signs.includes(value)?current.activity.signs.filter(item=>item!==value):[...current.activity.signs,value];return{...current,activity:{status:signs.length?'observed':'none',signs}}});return <section>
  ...
  <div className="fixed-context"><span>Zielfisch</span><strong>{fish==='pike'?'Hecht':'Barsch'}</strong><span>Gewässer</span><strong>See</strong></div>
  <div className="form-grid">
    ...
    <fieldset><legend>Aktivitätsanzeichen <small>mehrfach möglich</small></legend><div className="chips"><button type="button" className={conditions.activity.status==='unknown'?'selected':''} onClick={()=>setActivityStatus('unknown')}>Nicht geprüft</button><button type="button" className={conditions.activity.status==='none'?'selected':''} onClick={()=>setActivityStatus('none')}>Nichts sichtbar</button>{activityOptions(fish).map(([value,label])=><button type="button" key={value} className={conditions.activity.signs.includes(value)?'selected':''} onClick={()=>toggleActivity(value)}>{label}</button>)}</div></fieldset>
    <fieldset><legend>Weitere sichtbare Struktur <small>optional, mehrfach</small></legend><div className="chips">{structures(fish).map(([value,label])=><button type="button" key={value} className={conditions.observedStructure.includes(value)?'selected':''} onClick={()=>toggleStructure(value)}>{label}</button>)}</div></fieldset>
    {fish==='pike'&&<fieldset className="safety-check"><legend>Hechtsicher vorbereitet</legend><p>Erforderlich: hechtsicheres Vorfach, geeigneter Kescher, lange Lösezange und Abhakmöglichkeit. Schonzeit, Mindestmaß und Gewässerordnung vor Ort prüfen.</p><label><input type="checkbox" checked={conditions.pikeSafetyConfirmed===true} onChange={event=>setConditions(current=>({...current,pikeSafetyConfirmed:event.target.checked}))}/> Ich habe Ausrüstung und örtliche Regeln geprüft.</label></fieldset>}
  </div><button className="primary sticky-action" disabled={fish==='pike'&&!conditions.pikeSafetyConfirmed} onClick={()=>navigate('/empfehlung',{state:conditions})}>Drei Empfehlungen berechnen →</button>
</section>}
