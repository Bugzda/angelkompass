...
export function WaterCardPage() {
  ...
  if (!session) return <section><h1>Session nicht gefunden</h1><p>Die Session wurde möglicherweise gelöscht.</p><Link className="primary" to="/verlauf">Zum Verlauf</Link></section>
  ...
  return <section className="water-view"><CompactRecommendation recommendation={session.recommendation} fish={session.conditions.targetFish} progress={session.progress}/>
    {canFeedback&&<div className="water-feedback" aria-label="Rückmeldung erfassen"><button onClick={()=>feedback('bite')}>Biss</button><button onClick={()=>feedback('catch')}>Fang</button><button onClick={()=>feedback('no_success')}>Kein Erfolg →</button></div>}
