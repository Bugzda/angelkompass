...
function SessionDetails({ session }: { session: FishingSession }) {
  ...
  return <>
    ...
    <Link className="primary water-card-link" to={`/session/${session.id}/karte`}>Am-Wasser-Karte öffnen →</Link>
    ...
    <div className="attempts">{session.recommendation.switchPlan.map((step, index) => {
      ...
      return <article className={state} key={step.phase}><span className="step">0{index + 1}</span><div><h3>{step.title}</h3><p>{step.change}</p><small><b>{step.limit}</b> · {step.reason}</small></div></article>
    })}</div>
...
export function SessionPage() {
  ...
  if (!session) return <section><h1>Session nicht gefunden</h1><Link className="primary" to="/verlauf">Zum Verlauf</Link></section>
  return <section><p className="eyebrow">SESSION VOM {date(session.createdAt).toUpperCase()}</p><h1>Dein Versuch am Wasser.</h1>{error && <p className="storage-error">{error}</p>}<SessionDetails session={session} /></section>
}
